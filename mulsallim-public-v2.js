(()=>{
 ...
 document.querySelector('#go').onclick=()=>{if(!p.value||!c.value){alert('시·도와 시·군·구를 모두 선택해 주세요.');return}location.href=`mulsallim-dashboard-practice.html?province=${encodeURIComponent(p.value)}&city=${encodeURIComponent(c.value)}`};
 try{const favs=JSON.parse(localStorage.getItem('mulsallim-favorites')||'[]'),recent=JSON.parse(localStorage.getItem('mulsallim-recent')||'null'),all=[];if(recent)all.push(recent);favs.forEach(x=>{if(!all.some(y=>y.code===x.code))all.push(x)});if(all.length){const root=document.querySelector('#saved'),list=document.querySelector('#savedList');root.hidden=false;list.innerHTML=all.map(x=>`<a href="mulsallim-dashboard-practice.html?code=${encodeURIComponent(x.code)}&name=${encodeURIComponent(x.name)}&place=${encodeURIComponent(x.place)}">${x.name} · ${x.place}</a>`).join('')}}catch{}
})();
